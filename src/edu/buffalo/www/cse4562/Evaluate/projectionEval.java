...
public class projectionEval {
    ...
    public List<Column> usefulCol(Expression where, List<OrderByElement> orderBy, List<Column> groupByColumnReference, List<TableObject> involvedTables) {
        ...
        if (where != null) {
            ...
            result.addAll(selectionEval.parseSelect());
        }
            ...
            if (!table.toString().equals("null")){
                ...
                if (join.getRightNode() instanceof RATable) {
                    ...
                    if (tableName.equals(rightName)||tableName.equals(rightAlisa)) {
                        rightTable.addItemIntoColInf(c);
                        continue;
                    }
                }
                if (join.getLeftNode() instanceof RATable) {
                    ...
                    String leftAlisa = leftTable.getTable().getAlias().toUpperCase();
                    if (tableName.equals(leftName)||tableName.equals(leftAlisa)) {
                        leftTable.addItemIntoColInf(c);
                        continue;
                    }
                }
                if (join.getLeftNode() instanceof RASubSelect) {
                    RASubSelect leftTable = (RASubSelect) join.getLeftNode();
                    String leftName = leftTable.getSubSelectName().toUpperCase();
                    if (tableName.equals(leftName)) {
                        leftTable.addItemIntoColInf(c);
                        continue;
                    }
                }
                if (join.getRightNode() instanceof RASubSelect) {
                    RASubSelect rightTable = (RASubSelect) join.getRightNode();
                    String rightName = rightTable.getSubSelectName().toUpperCase();
                    if (tableName.equals(rightName)) {
                        rightTable.addItemIntoColInf(c);
                        continue;
                    }
                }
                ...
                if (involvedTables.size()==1){
                    ...
                    for (int j = 0;j<involvedTables.size();j++){
